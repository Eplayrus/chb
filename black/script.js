(() => {
  ...
  function saveMemory() {
    try {
      ...
      console.warn("Eva memory save failed:", error);
    }
  }
...
async function openFunctionWithMetrika(event) {
  event.preventDefault();
  ...
  url.searchParams.set("target", "cpa_approved");
  url.searchParams.set("price", "0");
  url.searchParams.set("currency", "RUB");
  url.searchParams.set("sub_ts", String(getCurrentUnixTime()));
  url.searchParams.set("comment", "black_button_click");

  if (button?.dataset?.redirect) {
    url.searchParams.set("redirect", button.dataset.redirect);
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", openFunctionWithMetrika);
  });
});
